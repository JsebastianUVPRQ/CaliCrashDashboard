...
from src.mapa import build_accident_deck, prepare_deck_map_data
...
def test_build_accident_deck_returns_heatmap_and_scatter_layers() -> None:
    result = build_accident_deck(_sample_accidents(), show_heatmap=True)

    assert isinstance(result, pdk.Deck)
    assert [layer.__class__.__name__ for layer in result.layers] == ["Layer", "Layer"]
    assert [layer.type for layer in result.layers] == ["HeatmapLayer", "ScatterplotLayer"]


def test_build_accident_deck_can_disable_heatmap() -> None:
    result = build_accident_deck(_sample_accidents(), show_heatmap=False)

    assert [layer.type for layer in result.layers] == ["ScatterplotLayer"]
